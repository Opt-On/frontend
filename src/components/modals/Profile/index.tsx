...
export const Profile: React.FC<ProfileProps> = ({ handleClose }) => {
  const { user, userCourseInfo, logout, avatar, updateAvatar } = useAuth();
  ...
  const Body = () => (
    <Box className={styles.body}>
          ...
          >
            {Array.from({ length: 7 }).map((_, index) => (
              <Box
                ...
                border="solid 2px"
                borderColor={
                  indexA === index ? "rgba(110, 119, 129, 0.5)" : "transparent"
                }
            ...
            {Array.from({ length: 7 }).map((_, index) => (
              <Box
                ...
                style={{ cursor: "pointer" }}
                color={
                  indexB === index ? activeColors[index] : inactiveColors[index]
                }
                border="solid 2px"
                borderColor={
                  indexB === index ? inactiveColors[index] : "transparent"
                }
      ...
      <Box display="flex" flexDirection="column">
        ...
        >
          <Box className={styles.flexColumn}>
            ...
            <TextInput
              value={userCourseInfo?.userInfo?.firstName || ""}
              disabled
              block
              style={{ cursor: "auto" }}
            />
          ...
          <Box className={styles.flexColumn}>
            ...
            <TextInput
              value={userCourseInfo?.userInfo?.lastName || ""}
              disabled
              block
              style={{ cursor: "auto" }}
            />
          ...
          <Box className={styles.flexColumn}>
            ...
            <TextInput
              value={userCourseInfo?.userInfo?.program || ""}
              disabled
              block
              style={{ cursor: "auto" }}
            />
      ...
      <Box display="flex" flexDirection="column">
        ...
        {userCourseInfo?.userInfo?.uploadDate && (
          <Text fontSize={14} color="#656d76">
            Last uploaded on {userCourseInfo?.userInfo.uploadDate}
          </Text>
        )}
        <FileUpload
          file={file}
